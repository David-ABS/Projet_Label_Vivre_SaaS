...
if st.session_state.page == 'accueil':
    ...
    with col2:
        if st.button("  Administrateur", use_container_width=True, type="primary"):
            st.session_state.profil = 'admin'
            st.session_state.page = 'dashboard'
            st.rerun()

        st.markdown("<br>", unsafe_allow_html=True)

        if st.button("  Établissement de santé", use_container_width=True):
            st.session_state.profil = 'etablissement'
            st.session_state.page = 'dashboard'
            st.rerun()

        st.markdown("<br>", unsafe_allow_html=True)

        if st.button("  Résident", use_container_width=True):
            st.session_state.profil = 'resident'
            st.session_state.page = 'dashboard'
            st.rerun()
...
# PAGE DASHBOARD
    ...
    with col_nav3:
        if st.button(" Export", use_container_width=True):
            ...
            st.rerun()
    with col_nav4:
        profil_label = {"admin": "Administrateur", "etablissement": "Établissement", "resident": "Résident"}.get(st.session_state.profil, "")
    ...
    with col_right:
        ...
        fig_pie = go.Figure(data=[go.Pie(
            ...
            hole=0.4,
            marker_colors=['#6BBFB5', '#F5A623', '#E8706A']
        )])
        fig_pie.update_layout(
            title='Répartition des répondants',
            title_font_family='Georgia',
            paper_bgcolor='white',
            height=350,
    ...
    with col_g1:
        fig_genre = px.bar(
            ...
            y='score_moyen',
            color='genre',
            color_discrete_sequence=['#E8706A', '#6BBFB5'],
            text='score_moyen',
            title='Score par genre'
        )
    ...
    with col_g2:
        ...
        fig_top = px.bar(
            ...
            orientation='h',
            color='score_moyen',
            color_continuous_scale=['#F5A623', '#6BBFB5'],
            title='Top 10 meilleures questions'
        )
...
# PAGE DONNÉES BRUTES
    ...
    with col_nav1:
        if st.button(" Tableau de bord", use_container_width=True):
            ...
            st.rerun()
    with col_nav2:
        if st.button(" Données brutes", use_container_width=True, type="primary"):
            pass
    with col_nav3:
        if st.button(" Export", use_container_width=True):
            st.session_state.page = 'export'
...
# PAGE EXPORT
    ...
    with col2:
        ...
        st.download_button(
            ...
            data=csv_public,
            file_name="scores_par_public.csv",
            mime="text/csv",
            use_container_width=True
        )
